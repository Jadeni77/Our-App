...
- **Module launcher** — the shell's generic mechanism for mounting modules: the **Games tab's springboard grid** — app tiles plus user-arranged **collections** (iOS-folder style) with full jiggle-mode editing, layout kept per device in a small JSON document (P11, supersedes P8's trailing rail; doc: `docs/modules/games-springboard.md`); tapping a tile mounts that module's entry view. Food Decision is app #1. *(In build — the rail ships until the tab shell lands.)*
...
| Games springboard (launcher surface — core shell chrome, documented module-style) | 🚧 Spec'd 2026-07-29, in build | `docs/modules/games-springboard.md` |
