...
- **Module launcher** — the shell's generic mechanism for mounting modules: the **Games tab's springboard grid** — app tiles plus user-arranged **collections** (iOS-folder style) with full jiggle-mode editing, layout kept per device in a small JSON document (P11, supersedes P8's trailing rail; spec: `docs/superpowers/specs/2026-07-29-tab-shell-design.md`); tapping a tile mounts that module's entry view. Food Decision is app #1. *(In build — the rail ships until the tab shell lands.)*
